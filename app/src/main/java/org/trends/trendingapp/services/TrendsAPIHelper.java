...
public class TrendsAPIHelper {


    private static final String TAG = TrendsAPIHelper.class.getSimpleName();

    public static void getTrendsPosts(final Context context) {

        RetrofitInterface retrofitInterface = BaseClient.getRetrofitInterface();
        retrofitInterface.getTrends( new Callback<TrendList>() {
            @Override
            public void success(TrendList postsDataWrapper, Response response) {

                try {

                    RealmList<Trends> realmList = postsDataWrapper.getData();
    ...
    public static class PostsInfoSuccess {
        ...
        public RealmList<Trends> postsDataRealmList;

        public PostsInfoSuccess(RealmList<Trends> mediaInfo) {
            this.postsDataRealmList = mediaInfo;
        }

        public RealmList<Trends> getPostsInfo() {
            return postsDataRealmList;
        }

        public void setPostsInfo(RealmList<Trends> mediaInfo) {
            this.postsDataRealmList = mediaInfo;
        }
    }
